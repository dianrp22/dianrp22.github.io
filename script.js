...
let getRules = function (comp, user) {
	...
	if (user === 'jempol' && comp == 'telunjuk') {
		return "Menang"
	}
	if (user === 'telunjuk' && comp == 'jempol') {
		return "Kalah"
	}
	if (user === 'jempol' && comp == 'kelingking') {
		return "Kalah"
	}
	if (user === 'kelingking' && comp == 'jempol') {
		return "Menang"
	}
	if (user === 'telunjuk' && comp == 'kelingking') {
		return "Menang"
	}
	if (user === 'kelingking' && comp == 'telunjuk') {
		return "Kalah"
	}
...
let proses = function () {
	...
	const gambar = ['jempol2', 'telunjuk2', 'kelingking2'];
	let i =0;
